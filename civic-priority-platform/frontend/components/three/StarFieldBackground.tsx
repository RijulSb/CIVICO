...
export default function StarfieldBackground() {
  ...
  useEffect(() => {
    ...
    // 1x1 transparent placeholder so the halo sampler is always valid
    ...
    const onScroll = () => computeScrollTarget();
    ...
    const updatePointer = () => {
      ...
      if (pointerActive) {
        ...
        if (Math.abs(dir.z) > 1e-4) {
          ...
          if (tHit > 0 && isFinite(tHit)) {
            target = raycaster.ray.origin
              .clone()
              .add(dir.clone().multiplyScalar(tHit));
          }
        }
      }
      ...
      const want = pointerActive && idle < 3 ? 1 : 0;
    ...
    const animate = () => {
      ...
      material.uniforms.uDrift.value +=
        dt * (CONFIG.drift + scrollCurrent * CONFIG.scrollDrift);

      const m = mouseSmooth;
      camera.position.set(m.x * 0.6, m.y * 0.6, 5 - scrollCurrent * 8);
      ...
      material.uniforms.uOpacity.value = fade * 2;

      group.rotation.z +=
        dt * (CONFIG.spin + scrollCurrent * CONFIG.scrollSpin);
